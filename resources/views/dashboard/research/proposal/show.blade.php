        ...
        <div class="col-md-6">
            <div class="tile mb-0">
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" disabled>{{ $proposal->title }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <ol>
                                    @foreach ($proposal->budgets as $budget)
                                        <li>{{ $budget->name }} (Rp {{ number_format($budget->cost, 0, ',', '.') }})
                                        </li>
                                    @endforeach
                                </ol>
                            </div>
                ...
                <div class="tile-footer d-flex justify-content-between">
                    ...
                    <div>
                        @canany(['internal'])
                            <a href="/{{ explode('.', Route::currentRouteName())[0] }}/{{ $proposal->research_id }}/edit"
                                class="btn btn-warning">
                                <i class="fa fa-pencil-square-o"></i>
                                Edit
                            </a>
                            &nbsp;&nbsp;&nbsp;
                            <form action="/{{ explode('.', Route::currentRouteName())[0] }}/{{ $proposal->research_id }}"
                                method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button onclick="return confirm('Yakin ingin membatalkan pengajuan proposal ini?')"
                        ...
                        @elsecan('external')
                            <form
                                action="/{{ explode('.', Route::currentRouteName())[0] }}/approve/{{ $proposal->research_id }}"
                                method="POST">
                                @csrf
                                @method('PUT')
